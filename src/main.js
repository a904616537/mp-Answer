import Vue     from 'vue'
import App     from './App'
import store   from './store'
import confirm from '@/components/confirm'
import wawa    from '@/components/wawa'
import share   from '@/components/share'

Vue.config.productionTip = false
App.mpType = 'app'

Vue.store                = store;
Vue.setting              = {
    api       : 'https://waguo.vip/',
    appid     : 'wxda1fdae90e92fe0a',
    secret    : '7e10eb9b3003db01b68b6c226c959e99',
    socketUrl : 'ws://106.14.112.101'
}

const app = new Vue(App)
app.$mount()

// 组件引用
Vue.component('confirm', confirm) //回答错误弹框
Vue.component('wawa', wawa) //赢取娃娃弹框
Vue.component('share', share) //转盘弹框

export default {
    // 这个字段走 app.json
    config : {
        // 页面前带有 ^ 符号的，会被编译成首页，其他页面可以选填，我们会自动把 webpack entry 里面的入口页面加进去
        pages  : ['pages/logs/main', '^pages/index/main', 'pages/start/main', 'pages/share/main'],
        window : {
            backgroundTextStyle          : 'light',
            navigationBarBackgroundColor : '#eefcff',
            navigationBarTitleText       : '答题赢娃娃',
            navigationBarTextStyle       : 'black'
        }
    }
}
